...
Compiles and passes `esphome config`/`esphome compile` against `crow_alarm_panel_test.yaml`. **Not yet validated on real hardware** — needs a fresh capture showing a mid-sequence CMD-byte change no longer aborts the sequence, and that it still resolves correctly (success or failure) via `ARMED_STATE`/watchdog alone.
