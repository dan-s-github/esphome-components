...
**Fix applied:** added `CrowAlarmPanel::last_confirmed_acp_state_`, updated alongside every controller-confirmed `alarm_control_panel_->publish_state()` call (the `ARMED_STATE` handler's Arming/Armed Away/Disarmed branches, and the "assume disarmed on zone motion"/"pending on zone alarm" heuristic branches) — i.e. everywhere except the optimistic `ACP_STATE_ARMING`/`ACP_STATE_DISARMING` publishes in `crow_alarm_control_panel.cpp`, which by design run ahead of confirmation. The arm/disarm watchdog's abort branch now republishes `last_confirmed_acp_state_` to `alarm_control_panel_`, restoring the entity to the last state the controller itself actually confirmed instead of leaving it stuck in the transitional state. Compiles and passes `esphome config`/`esphome compile` against `crow_alarm_panel_test.yaml`. **Not yet validated on real hardware** — needs a fresh capture reproducing a disarm-abort to confirm the entity recovers to `ACP_STATE_ARMED_AWAY` (allowing a retry) instead of staying stuck.
